...
@author: migdus
'''
import zipfile,tempfile,Tkinter, tkFileDialog, xml.etree.ElementTree as xml, os
...
class parser:
    ...
    def __init__(self,master):
        ...
        self.buttonDest = Tkinter.Button(frame,text='Destination',command=lambda:self.select_dir_path(select_dir_text, self.find_dest_dict_key,find_dir_entry_content))
        ...
        self.buttonStart = Tkinter.Button(frame,text='Start Parsing',command=lambda:self.start_parsing(csv_checked,plain_checked))
        self.buttonStart.grid(row=4,column=0,columnspan=2)

        self.scrollbar = Tkinter.Scrollbar(frame)
        self.scrollbar.pack(side=Tkinter.RIGHT, fill=Tkinter.Y)
        self.scrollbar.grid(row=5,column=0,columnspan=2)

        self.logbox = Tkinter.Listbox(frame, yscrollcommand=self.scrollbar.set)
        self.logbox.grid(row=5,column=0,columnspan=2)
    ...
    def start_parsing(self,csv_checked,plain_checked):
        ...
        for f in files:
            ...
            if fileExtension == '.xml': self.parse_xml(f)
    ...
    def parse_xml(self,xml_path):
        ...
        for element in rootElement.iter('outline'):
            ...
            self.gui_logger(element.attrib.keys())
